# Serverless API with AWS Lambda/Cognito/AWS Serverless Aurora (DataAPI)
...
## Deploy the ticket app application
...
```bash
Got src folder and run following commands to package and deploy applications locally, normally you don\'t need to do this as build is integrated with CI/CD pipline, it will do build and deployment automatically 
aws cloudformation package \
--template-file template.yml \
--s3-bucket aws-sam-ticketappdeployment \
--output-template-file out.yaml \
--s3-prefix ticketapp \
To deploy run following command

aws cloudformation deploy \
--template-file out.yaml \
--stack-name ticketapp \
...
Got to src folder
```bash
my-application$ sam local start-api --layer-cache-basedir opt
```
...
```bash
aws cloudformation delete-stack --stack-name ticketapp
```
